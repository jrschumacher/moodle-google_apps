...
global $CFG;
require_once($CFG->dirroot.'/blocks/gmail/library/store/OAuthStoreAbstract.class.php');
...
class OAuthStoreGoogle extends OAuthStoreAbstract {
    ...
    public function getSecretsForSignature ( $uri, $user_id )
    {
        ...
        $secrets = array();
        if ( !$rec = get_record('block_gmail_oauth_consumer_token','user_id',$user_id) ) {
    ...
    public function addServerToken ( $consumer_key, $token_type, $token, $token_secret, $user_id, $options = array() ) {
        ...
        if ( $rec = get_record('block_gmail_oauth_consumer_token', 'user_id', $user_id) ) {
            $data->id = $rec->id;
            update_record('block_gmail_oauth_consumer_token', $data);
        } else {
            insert_record('block_gmail_oauth_consumer_token', $data);
        }
    }
